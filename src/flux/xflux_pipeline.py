...
from src.flux.sampling import denoise, denoise_controlnet, get_noise, get_schedule, prepare, unpack, denoise_origin
...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False, model_path=None):
        ...
        if "fp8" in model_type:
            ...
            if model_path is None:
                ...
                self.model = load_flow_model_by_type(model_type, device="cpu" if offload else self.device, model_type=model_path)
    ...
    def set_lora(self, local_path: str = None, repo_id: str = None,
                 name: str = None, lora_weight: int = 0.7, is_diffusers=False):
        if is_diffusers:
            self.model = apply_lora_to_model(model=self.model, lora_path=local_path, lora_scale=lora_weight)
        else:
            checkpoint = load_checkpoint(local_path, repo_id, name)
